	...
	*/
	...
	{			
		...
		if($typeId == 1) {
			...
			$div = '<div class="well">
				<center>
					<h4>Attendance Type : Student &raquo;  Year: '.$year.' Month: '.$month.'<h4>
					<h4>Class : '.$classData['class_name'].' Section : '.$sectionData['section_name'].'<h4>
					
					<small>	
						<b>P</b> : Present | <b>A</b> : Absent | <b>L</b> : Late | <b>UN</b> : Undefined 
					</small>
				</center>
			</div>
							...
							for($i = 1; $i <= $numOfDays; $i++) {
								...
								foreach ($attendanceData as $attendanceKey => $attendanceValue) {
									if($attendanceValue['mark'] == 1) {
										...
										// absent
										$attendanceStatus = '<span class="label label-danger">A</span>';	
			...
			$div = '<div class="well">
				<center>
					<h4>Attendance Type : Teacher &raquo;  Year: '.$year.', Month: '.$month.'<h4>
