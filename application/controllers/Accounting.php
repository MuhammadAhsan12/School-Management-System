...
class Accounting extends MY_Controller 
{
	...
	public function fetchType($type = null)
	{
		if($type == 1) {
			...
			<div class="col-sm-6">
				...
				<table id="studentName" class="table table-hover table-bordered">
					<thead>
						<tr>
							<th>#</th>							
							<th>Name</th>							
	...
	*/
	public function fetchManageStudentPayData()
	{
		...
		foreach ($paymentData as $key => $value) {
			...
			if($value['status'] == 0) {
				$status = '<label class="label label-warning" style="font-size: 87%">Pending</label>';
			} else if($value['status'] == 1) {
				$status = '<label class="label label-success" style="font-size: 87%">Paid</label>';
			} else if($value['status'] == 2) {
				$status = '<label class="label label-danger" style="font-size: 87%">Unpaid</label>';
			}
	...
	*/
	public function fetchUpdatePaymentForm($type = null)
	{
		...
		if($type == 1) {
			...
			<div class="col-md-6">
				<table class="table table-hover table-bordered" id="studentData">
					<thead>
						<tr>
							<th>#</th>
							<th>Name</th>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">						
				<div id="remove-payment-message"></div>
					<table id="managePaymentTable" class="table table-hover table-bordered">
						<thead>
							<tr>
								<th>Name</th>
								<th>Start Date</th>
	...
	public function fetchManageStudentPayTable() 
	{
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">						
				...
				<table id="manageStudentPayTable" class="table table-hover table-bordered">
					<thead>
						<tr>
							<th>Payment Name</th>
							<th>Student Name</th>
	...
	*/	
	public function fetchExpensesDataForUpdate($id = null)
	{
        ...
        <table class="table table-bordered table-hover" id="editSubExpensesTable">
        <thead>
          <tr>
            <th>Name</th>
            <th>Amount</th>
        ...
        <tbody>';
        	...
        	foreach ($expensesItemData as $key => $value) {
        		$table .= '<tr id="row'.$x.'">
		            ...
		            <td>
		            <button type="button" class="btn btn-danger" onclick="removeEditExpensesRow('.$x.')"><i class="glyphicon glyphicon-remove"></i></button>
		            </td>
		          </tr>';
        	$x++;	
        	} // /.foreach          
	...
	*/
	public function viewIncomeDetail($paymentId = null)
	{
		if($paymentId) {
			...
			$data = '<table class="table table-bordered table-hover table-responsive table-striped">
			<tbody>
				<tr>
					<th>Payment Name : </th>
